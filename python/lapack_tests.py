...
class GMatrix:
    ...
    def __init__(self,mat,rows=None,cols=None,Blo=None,Bup=None,storage='full'):
        ...
        def loc_full((i,j)):         
            return (i,j)
        def loc_upper_banded((i,j)):
            if i<j: return (self.bup-j+i,j)
            else:   return (self.bup-i+j,i)

        if   self.store=='full': self.loc=loc_full
        elif self.store=='upper_banded': self.loc=loc_upper_banded
        else: sys.exit('loc not defined for storage= '+self.store)

    def __getitem__(self,ij):
        """return matrix element self[i,j]"""
        return self.m[self.loc(ij)]

    def __setitem__(self,ij,val): 
        """assign to matrix element self[i,j]=val"""
        self.symm=None # assigement may destroy symmetry
        self.m[self.loc(ij)]=val
    ...
    def bandwidth(self):
        ...
        if storag!='full': sys.exit('band-width determinantion only for storage = "full"')
    ...
    def restore(self,storage):
        """copy matrix into the given storage format
        """
