...
\end{equation}
 Where, $\psi$ is an eigenstate of $H$ of energy $E_0$ and $z\neq E_0$ is a complex number. Thus, we define the Green's operator as the matrix,
...
Let the bound states (indexed by $a$) of the Hamiltonian $H$ be denoted by $|\varphi_a\rangle$.
The potential is modified by the operator $\Gamma$ given by,
\begin{equation}\label{potmod}
  \Gamma=\displaystyle\sum_a |\varphi_a\rangle\langle\varphi_a|
\end{equation}
The above operator is multiplied by a  positive factor $\lambda$ which controls
the rate of convergence of the Born series. It is
...
\begin{align}
 \widetilde{H}&=H+\lambda\Gamma\\
 \widetilde{H}_0&=H_0+\lambda\Gamma
\end{align}
...
\subsubsection{Calculating Original Green's Operator}
The original Green's operator is related to the modified Green's operator through the following
Lippmann-Schwinger equation,
\begin{equation}\label{lippmannschwinger2}
 G=\widetilde{G}-\lambda\widetilde{G}\Gamma G
\end{equation}
We can then use the fact that,
\begin{equation}
 \widetilde{\Gamma}\equiv G(z)\Gamma=\Gamma G(z) = \displaystyle\sum_a \frac{|\varphi_a\rangle\langle\varphi_a|}{z-E_a} 
\end{equation}
where, $E_a$ is the energy of the bound state $|\varphi_a\rangle$. The above operator does not diverge
since $Re(z)>0$ and $E_a<0$. Denoting the $n^{th}$ order
approximation of $\widetilde{G}$ in \eqref{BornSeriesModCompact} as $\widetilde{G}^{(n)}$ we get,
\begin{equation}\label{Gapprox}
 G^{(n)}\approx \widetilde{G}^{(n)}+\lambda\widetilde{G}^{(n)}\widetilde{\Gamma}
\end{equation}

\subsubsection{$\lambda\rightarrow\infty$ Limit}
Before we go on to examples where we apply the orthogonal projection method, we will briefly review the behaviour of
the above technique in the $\lambda\rightarrow\infty$ limit as described in \cite{KukPom78}. The Lippmann-Schwinger equation \eqref{lippmannschwinger2} can also be written as,
\begin{equation}\label{lippmannschwinger3}
 \widetilde{G}=G+\lambda G\Gamma\widetilde{G}
\end{equation}
Where we have obtained an iterative expression for $\widetilde{G}$ in terms of $G$. From this we immediately obtain,
\begin{equation}
 \widetilde{G}=G+G\Gamma\lambda\left(\Gamma\left(I-\lambda G\right)\Gamma\right)^{-1}\Gamma G
\end{equation}
The admixture nonorthogonal to $\Gamma$ is,
\begin{equation}
 \Gamma\widetilde{G}=\Gamma\left(\Gamma\left(I-\lambda G\right)\Gamma\right)^{-1}\Gamma G \displaystyle\xrightarrow[\lambda\rightarrow\infty]{} 0,
\end{equation}
i.e., at $\lambda\rightarrow\infty$, the $\widetilde{G}$ is orthogonal to the subspace of the bound states
\begin{equation}
 \widetilde{G}=G-G\Gamma\left(\Gamma G\Gamma\right)^{-1}\Gamma G
\end{equation}
Where the inverse is taken only in the subspace of the bound states. The above expression thus reduces to,
\begin{equation}
 \widetilde{G}\rightarrow \left(I-\Gamma\right)G = G-\displaystyle\sum_i\frac{|\varphi_i\rangle\langle\varphi_i|}{E-E_i}
\end{equation}
Effectively, this means that the poles corresponding to the bound states have been eliminated from the original
Green's operator. This limit was not applied in the numerical examples because of the high computational 
overhead required. The above expression would have to be calculated separately for every energy, whereas for a finite 
$\lambda$, only one call is required for all scattering energies.
...
\subsubsection{Description of Plot}
We now plot the elements of the Born Series against the number of iterations for different scattering energies, $E_n$. In order
...
on the right. The x-axis denotes the number of iterations of the Born series, and the y-axis the matrix element of the
Green's operator, $\langle\mathbf{k}|G(E_n+i\epsilon)|\mathbf{k}\rangle$ ($|\mathbf{k}\rangle$ defined in \eqref{cosevec}) in log-scale. 
We follow this convention in the next case as well.

\begin{figure}[hb]
\includegraphics[width=320pt, height=135pt]{series1a.png}
\includegraphics[width=320pt, height=135pt]{series1d.png}
\end{figure}
\begin{figure}[ht]
%\includegraphics[width=320pt, height=135pt]{series1a.png}
%\includegraphics[width=320pt, height=135pt]{series1d.png}
\includegraphics[width=320pt, height=140pt]{series1b.png}
\includegraphics[width=320pt, height=140pt]{series1c.png}
\caption[\textwidth]{First Case - Weak Finite Well}
\end{figure}

\subsection{Second Case}
The second test potential is a mixture of a finite-well and a gaussian. Its plot is 
given below. We set $\epsilon=10^{-6}$ and $\lambda=100$.
\begin{figure}[H]
\centering
\includegraphics[width=215pt, height=135pt]{potential1d2.png}
\caption[\textwidth]{Test Potential 2}
\end{figure}
%\begin{figure}
%\includegraphics[width=320pt, height=135pt]{series3a.png}
%\end{figure}
\begin{figure}[ht]
\includegraphics[width=320pt, height=135pt]{series3a.png}
\includegraphics[width=320pt, height=135pt]{series3b.png}
\includegraphics[width=320pt, height=135pt]{series3c.png}
\includegraphics[width=320pt, height=135pt]{series3d.png}
\caption[\textwidth]{Finite Well + Gaussian}
\end{figure}
